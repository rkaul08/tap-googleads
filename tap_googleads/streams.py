...
class CampaignPerformance(ReportsStream):
    ...
    def gaql(self):
        return f"""
    SELECT campaign.name, campaign.status, campaign.advertising_channel_type, segments.device, segments.date, metrics.impressions, metrics.interactions, metrics.clicks, metrics.ctr, metrics.average_cpc, metrics.cost_micros FROM campaign WHERE segments.date >= {self.start_date} and segments.date <= {self.end_date}
    """
