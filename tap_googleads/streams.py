...
class CampaignPerformance(ReportsStream):
    ...
    def gaql(self):
        return f"""
    SELECT campaign.name, campaign.id, campaign.status, campaign.advertising_channel_type, segments.device, segments.date, metrics.impressions, metrics.interactions, metrics.clicks, metrics.ctr, metrics.average_cpc, metrics.cost_micros FROM campaign WHERE segments.date >= {self.start_date} and segments.date <= {self.end_date}
    """
...
class CampaignPerformanceByLocation(ReportsStream):
    ...
    def gaql(self):
        return f"""
    SELECT campaign_criterion.location.geo_target_constant, campaign.name, campaign_criterion.bid_modifier, segments.date, metrics.clicks, metrics.impressions, metrics.ctr, metrics.average_cpc, metrics.cost_micros FROM location_view WHERE segments.date >= '2021-01-01' and segments.date <= {self.end_date}
    """
